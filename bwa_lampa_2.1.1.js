...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function component(object) {
    ...
    function bwaChooseSeriesVariant(group, getChoiceFn) {
      ...
      var voice = bwa_voice_filter || choice.voice_name || '';
      if (voice) {
    ...
    this.createBwaToolbar = function() {
      ...
      var toolbar = $('<div class="bwa-ui">' +
        ...
        '</div>' +
        '<div class="bwa-ui__hint">' + (serial ? 'Выберите озвучку и серию — OK для просмотра' : 'Выберите вариант — OK для просмотра') + '</div>' +
      '</div>');
    ...
    this.setDefaultQuality = function(data) {
      ...
      var picked = pickQuality(data.quality, getPreferredQuality());
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      version: '2.1.1',
      name: 'BwaRC',
      description: 'BWA online: прямой вход из кнопки Смотреть, сезоны/серии, озвучки, источники и качество, 1080p по умолчанию',
      component: 'bwarch',
    ...
    function openBwaActivity(movie) {
      resetTemplates();
      Lampa.Component.add('bwarch', component);
    ...
    function openBwaFromFull(movie) {
      if (!movie) {
        try {
          var active = Lampa.Activity.active();
          if (active && active.component == 'full') {
            movie = active.card || (active.activity && active.activity.card);
          }
        } catch (err) {}
      }
      if (!movie) {
        Lampa.Noty.show('Не удалось определить фильм или сериал');
        return;
      }
      openBwaActivity(movie);
    }

    function addButton(e) {
      var render = e && e.render;
      if (!render || !render.length) return;

      var movie = (e && e.movie) || null;
      if (!movie && e && e.data) movie = e.data.movie;
      if (!movie) {
        try {
      ...
      // В штатной карточке Lampa кнопка «Смотреть» обычно имеет view--torrent.
      // Ранее мы перехватывали view--online, из-за чего на части сборок
      // наша кнопка вообще не получала hover:enter. Теперь заменяем именно
      // штатную кнопку просмотра и оставляем её внешний вид без изменений.
      var original = render.find('.view--torrent').first();
      if (!original.length) original = render.find('.view--online').first();

      render.find('.bwa-direct-watch').remove();

      if (original.length) {
        var replacement = original.clone(false);
        replacement.addClass('bwa-direct-watch');
        replacement.attr('data-subtitle', 'BwaRC • открыть варианты просмотра');
        ...
        var run = function(evt) {
          ...
          openBwaFromFull(movie);
          setTimeout(function() { opened = false; }, 350);
        };

        replacement.on('hover:enter.bwa', run);
        replacement.on('click.bwa', run);
        original.replaceWith(replacement);
        return;
      }

      // Запасной вариант для сборок, где штатной кнопки просмотра нет.
      var btn = $(Lampa.Lang.translate(button));
      btn.addClass('bwa-direct-watch');
      btn.on('hover:enter.bwa click.bwa', function(evt) {
    ...
    try {
      ...
      if (active && active.component == 'full') {
        addButton({
          render: active.activity && active.activity.render ? active.activity.render() : $(active.render || []),
          movie: active.card || (active.activity && active.activity.card)
        });
      }
    } catch (e) {}
