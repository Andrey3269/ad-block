...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function startPlugin() {
    ...
    function addButton(e) {
      ...
      var movie = e.movie;
      var online = render.find('.view--online').first();
      render.find('.lampac--button').remove();
      if (online.length && !online.hasClass('bwa-direct-watch')) {
        var replacement = online.clone(false);
        replacement.addClass('bwa-direct-watch');
        replacement.attr('data-subtitle', 'BwaRC • сразу открыть варианты просмотра');
        replacement.on('hover:enter', function(evt) {
